...
{
    amrex::Print() << "IN MAKE NEW " << lbase << std::endl;
	...
	tags.collate(tagvec);
        ...
        if (tagvec.size() > 0)
        {
            ...
            if (ParallelDescriptor::IOProcessor()) {
                ...
                amrex::Print() << "NEW BX " << new_bx << std::endl;
                new_bx.simplify();
                BL_ASSERT(new_bx.isDisjoint());

            std::cout << "NEW BX BEFORE INTERSECT " << levf << " " << new_bx << std::endl;
            ...
            std::cout << "NEW BX HERE " << levf << " " << new_bx << std::endl;
    ...
    amrex::Print() << "LEAVING MAKE NEW " << lev << " " << new_grids[lev] << std::endl;
    }
}
